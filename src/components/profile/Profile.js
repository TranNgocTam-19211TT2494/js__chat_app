...
export default function Profile(props) {
  ...
  let { username, email, userImageUrl, _id } =
  ...
  const handleOkOfEditProfile = () => {
    ...
    axios
      .post("https://api.cloudinary.com/v1_1/coders-tokyo/upload", formData)
      .then((res) => {
        let newInfo = {
          _id,
        ...
        axios
          .post(
            `https://chat-app-server-linh.herokuapp.com/account/edit-profile/${_id}`,
            {
              userImageUrl: res.data.url,
              username: newUsername
            },
